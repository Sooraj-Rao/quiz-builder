...
const AdminQuizEdit = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/admin/quiz/${testId}`
      );
  ...
  const saveQuestion = async (question) => {
    try {
      await axios.put(
        `http://localhost:5000/api/admin/quiz/question/${question._id}`,
        {
          text: question.text,
  ...
  const deleteQuestion = async (questionId, qIndex) => {
    if (window.confirm("Are you sure you want to delete this question?")) {
      try {
        await axios.delete(
          `http://localhost:5000/api/admin/quiz/question/${questionId}`
        );
      ...
      <div className="card">
        <div className="card-header">
          <div className="flex justify-between align-center">
            <div>
              <h1 className="card-title">✏️ Admin Edit Quiz: {testId}</h1>
          ...
          <div key={question._id} className="card">
            <div className="flex justify-between align-center mb-20">
              ...
              <div className="flex gap-8 align-center">
                <span className="badge badge-warning">ADMIN EDIT</span>
                <button
